...
def otpt_orthogonality_loss(model):
    ...
    outer_products = v.unsqueeze(2) @ v.unsqueeze(1)
    transformed_matrix = e.unsqueeze(0).expand(Wwt.shape[0], -1, -1) - 2 * outer_products

    Hx = torch.bmm(transformed_matrix, Wwt.unsqueeze(2)).squeeze(2)
    Ht_ortho = Hx - e

    return torch.linalg.norm(Ht_ortho, dim=-1).mean()
...
def test_time_tuning(model, inputs, optimizer, scaler, args, reliable_mask=None):
    ...
    for j in range(args.tta_steps):
        ...
        loss = entropy_avg(output)
        if args.otpt:
            loss_otpt = otpt_orthogonality_loss(model)
            loss = loss + args.lambda_otpt * loss_otpt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_otpt', type=float, default=0.1,
